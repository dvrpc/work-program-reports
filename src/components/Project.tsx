...
import type { Project, FundingDetails, LRPImages, Amendment } from "src/Project";
...
export const ProjectView = ({
  ...
  projects: Queries.ProjectPageQuery["projects"] & (Project | Amendment);
  site: Queries.ProjectPageQuery["site"];
}) =>
  projects && (
    <article>
        ...
        <tbody>
          <tr className="font-bold border-b-[3px] border-black">
            <th className="text-left pr-8 whitespace-nowrap w-1">
              PROJECT: {projects.proid}
            </th>
            <th className="text-left">{projects.proname}</th>
          </tr>
          <tr>
            <th className="text-left pr-8 whitespace-nowrap w-1">
              Responsible Agency:
            </th>
            <td>{projects.respagency}</td>
          </tr>
          <tr>
            <th className="text-left pr-8 whitespace-nowrap w-1">
              Program Coordinator:
            </th>
            <td>{projects.coordinator}</td>
          </tr>
          <tr>
            <th className="text-left pr-8 whitespace-nowrap w-1">
              Project Manager(s):
            </th>
            <td>{projects.managers}</td>
          </tr>
          {projects.lrpimages && (
            <tr>
              <th className="text-left pr-8 whitespace-nowrap w-1">
              ...
              <td>
                <div className="inline-flex gap-1">
                  {Object.values(
                    (JSON.parse(projects.lrpimages) as LRPImages[])[0]
                  ).map(
                    (image) =>
                      image && (
                        <img
      ...
      <p dangerouslySetInnerHTML={{ __html: projects.goals ?? "" }} />
      ...
      <p dangerouslySetInnerHTML={{ __html: projects.tasks ?? "" }} />
      <h3 className="my-2 font-bold underline">Products:</h3>
      <p dangerouslySetInnerHTML={{ __html: projects.products ?? "" }} />
      ...
      <table className="w-full border-y-2 border-black">
        ...
        <tbody>
          {(JSON.parse(projects.funding_details ?? "[]") as FundingDetails[])
            .sort((a, b) => a.fy - b.fy)
            .map((row) => (
              <tr key={row.fy} className="border-y border-black">
      ...
      {projects.fundingnote && (
        <>
          <p>{site?.siteMetadata?.year ?? ""} Other Funding Details:</p>
          <p dangerouslySetInnerHTML={{ __html: projects.fundingnote }} />
        </>
      )}
    </article>
  );
